...
?>
            ...
            <div style="margin:20px 0 40px;">
                ...
                ?>
                <span class="description"><?php esc_html_e('Please note: If you have successfully registered the above Username and API Key and no updates are showing up '
                        . 'under the "Dashboard &rarr; Updates" page, wait a few minutes and refresh the page. It is because the API connection may be cached in the database.', 'udesign' ); ?></span>
            </div>
...
function field_udesign_theme_update_callback() {
	echo '<p>';
        printf( esc_html__('In order to be able to update the theme you need to provide your ThemeForest Username and API Key for the account that you purchased the theme from. '
                . 'You can obtain an API key by visiting the %s (make sure that you are signed in) then clicking the "My Settings" tab. '
                . 'At the bottom of the page you will find your account\'s API key and a button to regenerate a new one if needed.', 'udesign'), 
...
function input_theme_forest_username_callback() {
	$options = get_option('udesign_theme_update_options'); ?>
        ...
        <input type="text" name="udesign_theme_update_options[input_theme_forest_username]" id="input_theme_forest_username" value="<?php esc_attr_e($options['input_theme_forest_username']); ?>" size="32" />
...
function input_theme_forest_api_key_callback() {
	$options = get_option('udesign_theme_update_options'); ?>
        ...
        <input type="password" name="udesign_theme_update_options[input_theme_forest_api_key]" id="input_theme_forest_api_key" value="<?php esc_attr_e($options['input_theme_forest_api_key']); ?>" size="32" />
...
function u_design_alternative_update_methods_info() {
    ob_start(); ?>
            ...
            <p><?php printf( esc_html__('First you need to download the latest version of the %1$sU-Design Theme%2$s,'
                    . ' for that log into your %3$s account used to purchase the theme and from your %4$sDownloads%5$s section grab the theme\'s latest zip.', 'udesign'),
