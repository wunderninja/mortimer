...
if ( ! function_exists( 'udesign_google_fonts_url' ) ) {
    ...
    function udesign_google_fonts_url() {
        ...
        $fonts_url = '';
        $fonts = array_unique( $udesign_options['google_web_fonts_assoc'] );
...
function udesign_display_breadcrumbs() {
    ...
    if ( $udesign_options['show_breadcrumbs'] == 'yes' && ! $disable_breadcrumbs ) {
 ...
 * This will hide certain tabs under the Settings -> Visual Composer page as well as the updater license key nag
 */
...
function udesign_add_custom_editor_styles() {
    ...
    if ( ! $udesign_font_awesome_options['udesign_disable_font_awesome'] ) {
        add_editor_style( get_template_directory_uri() . '/styles/common-css/font-awesome/css/font-awesome.css' );
    }
}
...
function udesign_check_page_layout_option( $option_to_check = 'no_header' ) {
    $udesign_page_layout_options = get_post_meta( get_the_ID(), '_udesign_page_layout_options', true );
