...
<?php                           if ( udesign_is_fontello_installed() ) : ?>
                                    ...
                                    <div class="clear"></div>
                                    <span class="description" style="display:inline-block; margin:2px 0 10px"><?php esc_attr_e('Deleting or replacing the currently installed '
                                            . 'Fontallo icons would affect those icons used in your content, so don\'t forget to update your content accordingly.', 'udesign'); ?></span>
<?php                           endif; ?>
...
function field_fontello_uploader_callback() {
	echo '<p>'; ?>
        ...
        printf( esc_html__('Before you can use this section you will need to go to %s, '
                . 'choose the icon fonts you would like to include in the theme and then generate your zip file, '
                . 'upload the fontello zip file in the section below.', 'udesign'), 
...
function udesign_field_font_awesome_options_callback() {
	echo '<p>' . esc_html__('By default the Font Awesome fonts and styles are loaded within the theme. '
                . 'This means that the icons are available for your choosing in the page, post or anywhere else in your content. '
                . 'If you would like to remove Font Awesome from the available icon fonts, please use the option below to disable them:', 'udesign') . '</p>';
}
...
function icon_fonts_upload_button_callback() {
	$options = get_option('udesign_icon_fonts_options'); ?>
        ...
        <input type="hidden" name="udesign_icon_fonts_options[last_installed_fontello_filename]" id="last_installed_fontello_filename" value="<?php esc_attr_e($options['last_installed_fontello_filename']); ?>"  />
        ...
        if ( $fontello_zip_file ) {
            ...
            if( ! $wp_filesystem->is_dir( $to ) ) {
                ...
                if ( ! $wp_filesystem->mkdir( $to ) ) { ?>
                        ...
                        <div class="error"><p><?php _e( "uDesign Error: There was an error creating the '{$to}' folder", 'udesign' ); ?></p></div>
                        <?php
                }
    ...
    if ( $access_type === 'direct' ) {
        ...
        if( $wp_filesystem->is_dir( $fontello_folder ) ) {
            ...
            if ( ! $wp_filesystem->rmdir( $fontello_folder, true ) ) { ?>
                <div class="error"><p><?php _e( "uDesign Error: Deletion of '{$fontello_folder}' folder failed!", 'udesign' ); ?></p></div>
                <?php
                return false;
            }
...
function udesign_get_fontello_style_sheet( $css_file = 'fontello.css' ) {
    ...
    <div class="error"><p><?php _e( "uDesign Error: '{$css_file}' file could not be found.", 'udesign' ); ?></p></div><?php
    return false;
}
    ...
    if ( 'post.php' === $pagenow || 'post-new.php' === $pagenow || 'admin-ajax.php' === $pagenow ) {
            ...
            function udesign_load_icon_fonts_wp_admin_scripts() {
                    ...
                    if ( ! $udesign_font_awesome_options['udesign_disable_font_awesome'] ) {
                        wp_enqueue_style('u-design-font-awesome', get_template_directory_uri() . '/styles/common-css/font-awesome/css/font-awesome.css', false, UDESIGN_VERSION, 'screen');
                    }
                    ...
                    wp_enqueue_style('u-design-select2', get_template_directory_uri() . '/scripts/admin/select2/select2.css', false, '3.5.2', 'screen');
                    // load the "select2" scripts
                    wp_enqueue_script('u-design-select2', get_template_directory_uri() . '/scripts/admin/select2/select2.min.js', array('jquery'), '3.5.2', true);
